...
def legacy_to_profile_result(legacy_profile: Any) -> DatasetProfile:
    ...
    for c in legacy_profile.columns:
        ...
        metrics["null_pct"] = Metric(
            ...
            coverage=Coverage(rows_examined=total_c, rows_total=legacy_profile.num_rows),
            method="legacy",
            parse_errors=getattr(c, "type_mismatch_count", 0)
        )
        
        metric_map = {
            "mean": "mean", "val_min": "min", "val_max": "max", "std": "std",
            "min_str_len": "min_len", "max_str_len": "max_len"
        }
        for legacy_attr, can_attr in metric_map.items():
            if hasattr(c, legacy_attr):
                metrics[can_attr] = Metric(
                    ...
                    status=MetricStatus.EXACT if not is_sampled else MetricStatus.SAMPLED,
                    coverage=Coverage(rows_examined=getattr(c, "total_count", 0), rows_total=legacy_profile.num_rows),
                    method="legacy"
                )
        ...
        cp = ColumnProfile(
            ...
            exact_numeric_overflowed_val=bool(getattr(c, "exact_numeric_overflowed", False)),
    ...
    overall["null_pct"] = Metric(
        ...
        status=MetricStatus.EXACT if not is_sampled else MetricStatus.SAMPLED,
        coverage=Coverage(rows_examined=legacy_profile.num_rows, rows_total=legacy_profile.num_rows),
        method="legacy"
    )
