...
def test_f02_clean_executor_false_rollback(tmp_path):
    ...
    with patch("os.replace", side_effect=PermissionError("Simulated locked file error")):
        with pytest.raises(ZeddaError) as exc_info:
            execute_cleaning_transaction(
                df, plan, target_path=str(target_file),
                audit_actions=[], score_before=50, score_after=100
            )
