"""
fasteda Python tests — runs in CI after wheel build
"""
import sys
import os
import pytest
...
from fasteda import fasteda_core as core
...
# ── Tests ─────────────────────────────────────────────────────────
def test_version():
    assert fe.__version__ == "0.1.0"


def test_scan_returns_profile(sample_csv):
    p = fe.scan(sample_csv)
    assert p.num_rows == 5
    assert p.num_cols == 4
