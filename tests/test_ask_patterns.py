...
# ── Mock objects ──────────────────────────────────────────────────
class MockCol:
    def __init__(self, name, type_str, null_pct, null_count, unique_approx,
                 unique_pct=0.0, mean=0.0, stddev=0.0, val_min=0.0, val_max=0.0,
                 skewness=0.0, is_constant=False, has_high_nulls=False,
...
class MockProfile:
    ...
    is_sampled = False
    columns = [
        MockCol('PassengerId', 'int',   0.0,  0,   1000, unique_pct=100.0, mean=500.0, val_min=0,  val_max=999),
        MockCol('Name',        'str',   0.0,  0,   1000, unique_pct=100.0),
        MockCol('Pclass',      'int',   0.0,  0,   3,    unique_pct=0.3,   mean=2.1,   val_min=1,  val_max=3),
        MockCol('Survived',    'int',   0.0,  0,   2,    unique_pct=0.2,   mean=0.5,   val_min=0,  val_max=1),
        MockCol('Age',         'float', 6.5,  65,  74,   unique_pct=7.4,   mean=29.6,  val_min=1,  val_max=80,   skewness=0.4),
        MockCol('Fare',        'float', 0.0,  0,   300,  unique_pct=30.0,  mean=35.0,  val_min=5,  val_max=5000, skewness=3.8),
...
from zedda import (
    _ask_pattern_a, _ask_pattern_b, _ask_pattern_c, _ask_pattern_d,
    _ask_sanitize_question, _ask_validate_path,
)
...
check('D02: column count',
      _ask_pattern_d(p, 'how many columns does this have?'),
      must_contain=['7'])

check('D03: quality score',
      _ask_pattern_d(p, 'what is the data quality score?'),
      must_contain=['score'])

check('D04: outliers (Fare max=5000, mean=35 → 142x)',
      _ask_pattern_d(p, 'which columns have outliers?'),
      must_contain=['fare'])

check('D05: binary col (Survived 0/1)',
      _ask_pattern_d(p, 'binary columns?'),
      must_contain=['survived'])

check('D06: id column (PassengerId 100% unique)',
      _ask_pattern_d(p, 'which are the id columns?'),
      must_contain=['passengerid'])

check('D07: correlated columns',
      _ask_pattern_d(p, 'are there correlated columns?'),
      must_contain=['pclass', 'fare'])

check('D08: mean of Age',
      _ask_pattern_d(p, 'mean of Age'),
      must_contain=['29.6'])

check('D09: null rate of Cabin',
      _ask_pattern_d(p, 'null rate of Cabin'),
      must_contain=['75.0'])

check('D10: type of Survived',
      _ask_pattern_d(p, 'type of Survived'),
      must_contain=['int'])

check('D11: min of Fare',
      _ask_pattern_d(p, 'minimum of Fare'),
      must_contain=['5'])

check('D12: max of Fare',
      _ask_pattern_d(p, 'maximum of Fare'),
      must_contain=['5000'])

check('D13: drop suggestions (PassengerId ID + Cabin 75% null)',
      _ask_pattern_d(p, 'what should I drop?'),
      must_contain=['passengerid', 'cabin'])

check('D14: skewed columns (Fare skewness=3.8 > 2.0 for 1000 rows)',
...
for i, line in enumerate(src.split('\n'), 1):
    ...
    if 'groq' in stripped.lower() and 'groq' in stripped.lower():
        ...
        if 'api.groq.com' in stripped:
            continue
        if '_AI_PRICING' in stripped or '_AI_DEFAULT_MODEL' in stripped or '_ask_zedda_ai' in stripped:
            continue
